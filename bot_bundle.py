# ruff: noqa: E402, F811, F841, E741, F541  # Игнорируем предупреждения линтера, обусловленные объединением модулей в один файл.
# pyright: reportMissingImports=false  # Отключаем жалобы на отсутствующие модули, так как они представлены внутри монолита.
...
class AdvancedArbitrageEngine:
    ...
    def _initialize_data_structures(self):
        ...
        self.triangles_in_execution = set()
        self.ohlcv_history = {}
    ...
    def execute_arbitrage(self, opportunity):
        ...
        if triangle_name in self.triangles_in_execution:
            logger.debug(
                "🔄 Треугольник %s уже исполняется в текущем цикле, пропускаем повтор",
                triangle_name
            )
            return False

        self.triangles_in_execution.add(triangle_name)
        ...
        try:
            ...
            self.triangles_in_execution.discard(triangle_name)
