...
class OptimizedConfig:
    ...
    def __post_init__(self) -> None:
        """Подхватывает значения из переменных окружения с безопасным приведением типов."""

        self.LOG_LEVEL = os.getenv("LOG_LEVEL", self.LOG_LEVEL)
        self.LOG_FILE = os.getenv("LOG_FILE", self.LOG_FILE)
        self.TESTNET = self._get_bool_env("TESTNET", self.TESTNET)
        self.MIN_TRIANGULAR_PROFIT = self._get_float_env(
            "MIN_TRIANGULAR_PROFIT", self.MIN_TRIANGULAR_PROFIT
        )
        self.TRADE_AMOUNT = self._get_float_env("TRADE_AMOUNT", self.TRADE_AMOUNT)
        self.REPLAY_DATA_PATH = os.getenv("REPLAY_DATA_PATH", self.REPLAY_DATA_PATH)
        self.REPLAY_SPEED = self._get_float_env("REPLAY_SPEED", self.REPLAY_SPEED)
        self.REPLAY_MAX_RECORDS = self._get_int_env(
            "REPLAY_MAX_RECORDS", self.REPLAY_MAX_RECORDS
        )
        self.COOLDOWN_PERIOD = self._get_int_env(
            "COOLDOWN_PERIOD", self.COOLDOWN_PERIOD
        )
        self.MARKET_SNAPSHOT_SYMBOLS = self._get_int_env(
            "MARKET_SNAPSHOT_SYMBOLS", self.MARKET_SNAPSHOT_SYMBOLS
        )

    @staticmethod
    def _get_bool_env(key: str, default: bool) -> bool:
        """Безопасно получает булеву настройку из окружения."""
    ...
    def _get_float_env(key: str, default: float) -> float:
        """Преобразует значение окружения в float, сохраняя дефолт при ошибке."""
    ...
    def _get_int_env(key: str, default: Optional[int]) -> Optional[int]:
        """Возвращает целое значение из окружения или дефолт при невозможности парсинга."""
